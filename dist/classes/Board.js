...
class Board {
    constructor(zombies, creatures, dimension) {
        this.initializeBoard = () => {
            ...
            for (let i = 0; i < this.creatures.length; i++) {
                ...
                let row = boardMap.get(position[1]);
                row[position[0]] = `C`;
            }
            // Print Board to terminal
            console.log("Here is the board with the Zombie and Creature positions:", boardMap);
            console.log("------------------------------------------------");
    ...
    updatePosition(boardMap, zombie) {
        ...
        console.log("------------------------------------------");
        console.log(`zombie ${zombie.index} moved to (${zombie.newZombieXPosition}, ${zombie.newZombieYPosition})`);
    }
    checkForCreature(boardMap, zombie) {
        // Get the row in the hashmap
        let row = boardMap.get(zombie.newZombieYPosition);
        ...
        if (creatureEncounter.toLowerCase() === "c") {
            ...
            const zombieToAdd = new Zombie_1.Zombie(newZombiePosition, zombie.moves, zombie.index + 1);
            this.zombies.push(zombieToAdd);
            console.log(`zombie ${zombie.index} infected creature at (${zombie.newZombieXPosition} , ${zombie.newZombieYPosition})`);
        }
    }
    finalOutput() {
        ...
        for (let i = 0; i < this.zombies.length; i++) {
            console.log(`zombie ${i} final positions: (${this.zombies[i].zombieXPosition},${this.zombies[i].zombieYPosition})`);
        }
        console.log("------------------------------------------");
    }
}
